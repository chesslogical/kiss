...
// Optimized for Linux with file permissions set to 0o600 on output files.
// On non-Unix platforms, permissions follow system defaults.
// This provides better security for sensitive encrypted/decrypted files on Unix-like systems.
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Cmd::Encrypt {
            ...
            no_confirm,
            force,
        } => encrypt_cmd(input, output, profile, chunk_mib, no_confirm, force),
        Cmd::Decrypt { input, output, force } => decrypt_cmd(input, output, force),
    }
}
...
fn encrypt_cmd(
    ...
    if output.exists() && !force {
        bail!("Output file '{}' exists. Use --force to overwrite.", output.display());
    }
    ...
    let file = open_options.open(&output).with_context(|| format!("create {}", output.display()))?;
    ...
    while remaining > 0 {
        ...
        // Update progress
        if total_len > 0 {
            let processed = (total_len as u128 - remaining) as u64;
            let percent = ((processed as f64 / total_len as f64) * 100.0) as u32;
        ...
        // Check for chunk index overflow
        idx = idx.checked_add(1).ok_or_else(|| anyhow!("File too large — chunk counter would wrap"))?;
    }
    ...
    println!("\nEncrypted (profile={:?}, chunk={} MiB) → {}", profile, chunk_mib, output.display());
    Ok(())
}

fn decrypt_cmd(input: PathBuf, output: PathBuf, force: bool) -> Result<()> {
    if input == output {
        bail!("Input and output paths must differ");
    }
    ...
    let mut writer_opt: Option<BufWriter<std::fs::File>> = None;
    ...
    while remaining > 0 {
        ...
        // Create writer if not yet created (after successful decrypt)
    ...
    // If no data was written (empty file), create the empty output file
    if writer_opt.is_none() {
        let mut open_options = OpenOptions::new();
        open_options.write(true).create(true).truncate(true);
        #[cfg(unix)]
        open_options.mode(0o600);
        open_options.open(&output).with_context(|| format!("create empty {}", output.display()))?;
        // File is created and closed, empty
    } else {
        writer_opt
            .unwrap()
            .flush()
...
fn derive_key(
    ...
    let m_cost_kib: u64 = (mem_mib as u64)
        .checked_mul(1024)
        .context("Argon2 memory parameter overflows")?;
    if m_cost_kib > u32::MAX as u64 {
        bail!("Argon2 memory cost exceeds library limit");
    }

    let params = Params::new(m_cost_kib as u32, time_cost, lanes, Some(32))
