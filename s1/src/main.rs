...
const HEADER_SIZE: usize = MAGIC_HEADER.len() + 1;
const NONCE_SIZE: usize = 12;
const TAG_SIZE: usize = 16;
const HASH_SIZE: usize = 32;
const KEY_SIZE: usize = 32;
const KEY_FILE: &str = "1.key";
...
fn main() -> Result<()> {
    ...
    let key_path = current_dir.join(KEY_FILE);

    if file_path == key_path {
        anyhow::bail!("Cannot operate on the key file itself.");
    }

    if !key_path.exists() {
        anyhow::bail!("Key file '{}' not found in current directory.", KEY_FILE);
    }
    if !file_path.exists() {
    ...
    let mut key_bytes = vec![0u8; KEY_SIZE];
    {
        ...
        let bytes_read = key_file.read(&mut key_bytes).context("Failed to read key file")?;
        if bytes_read != KEY_SIZE {
            anyhow::bail!("Key file must be exactly {} bytes.", KEY_SIZE);
        }
    }
    let cipher = Aes256GcmSiv::new_from_slice(&key_bytes).map_err(|_| anyhow::anyhow!("Invalid key length"))?;
    key_bytes.zeroize(); // Securely zero key from memory
    ...
    let result = if is_encrypted {
        ...
        let nonce_slice = &data[HEADER_SIZE..HEADER_SIZE + NONCE_SIZE];
        let nonce = Nonce::from_slice(nonce_slice);
        let ciphertext_with_tag = &data[HEADER_SIZE + NONCE_SIZE..];
        let payload = Payload {
