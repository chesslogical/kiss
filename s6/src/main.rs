...
const MAGIC_HEADER: &[u8] = b"TF256ENC"; // 8 bytes magic, changed for 256
const IV_SIZE: usize = 16;
const TAG_SIZE: usize = 32; // Block size for CMAC tag
const HEADER_SIZE: usize = MAGIC_HEADER.len() + IV_SIZE;
const KEY_FILE: &str = "key.bin"; // Expected key file name

#[derive(Parser)]
#[command(name = "threefish_encrypt", about = "CLI for Threefish-256 file encryption/decryption with auto-detect, in-place overwrite, and authentication")]
struct Args {
...
fn main() -> Result<()> {
    ...
    if key_bytes.len() != 32 {
        return Err(anyhow::anyhow!("Key file {} must be exactly 32 bytes", key_path.display()));
    }
    let key_array: [u8; 32] = key_bytes.try_into().map_err(|_| anyhow::anyhow!("Key conversion error"))?;
    // Read input file
    let mut data = vec![];
    ...
    let (output, is_decrypt) = if data.starts_with(MAGIC_HEADER) {
        ...
        // Verify MAC
        let mut mac = <Cmac<Threefish256> as KeyInit>::new(GenericArray::from_slice(&key_array));
...
fn process_ctr(key_array: &[u8; 32], iv_array: &[u8; 16], data: &[u8]) -> Result<Vec<u8>> {
    // Threefish-256 instance with key and tweak (IV)
    let cipher = Threefish256::new_with_tweak(key_array, iv_array);
    ...
    for i in 0..num_blocks {
        ...
        <Threefish256 as BlockEncrypt>::encrypt_block(&cipher, &mut ctr_generic);
